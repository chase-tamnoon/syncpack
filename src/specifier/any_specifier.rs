use super::{parser, semver::Semver, semver_range::SemverRange, Specifier};
...
impl AnySpecifier {
  ...
  pub fn with_range_if_semver(&self, range: &SemverRange) -> Self {
    let tree = Specifier::new(&self);
    if let Specifier::Semver(Semver::Simple(simple_semver_specifier)) = tree {
